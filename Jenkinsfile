pipeline {
    ...
    environment {
        ...
        IMAGE_NAME = 'web-back-members-k8s'
        GITHUB_REPO = 'https://github.com/jbnu-web-class-project/members.git'
                ...
                script {
                    sshagent(credentials: [SSH_CREDENTIALS_ID]) {
                        ...
                        script {
                            ...
                            cd k8s-manifest &&
                            ls -l &&
                            sed -i 's|image: .*\$|image: ${HARBOR_REPO}/heim/${IMAGE_NAME}:${env.BUILD_ID}|' members/back-members-deploy.yaml &&
                            git config --global user.email "gjdhks1212@gmail.com"
                            git config --global user.name "hodu26"
                            git add members/back-members-deploy.yaml &&
